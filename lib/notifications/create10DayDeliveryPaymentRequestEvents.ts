...
export const DELIVERY_PAYMENT_REQUEST_10_DAY_INTERVAL_DAYS = 10;
export const DELIVERY_PAYMENT_REQUEST_10_DAY_REQUESTED_ON_TIME = "09:19:00.000Z";

export const DELIVERY_PAYMENT_REQUEST_10_DAY_SKIP_REASONS = {
...
export type DeliveryPaymentRequest10DaySkipReason =
  (typeof DELIVERY_PAYMENT_REQUEST_10_DAY_SKIP_REASONS)[keyof typeof DELIVERY_PAYMENT_REQUEST_10_DAY_SKIP_REASONS];

type DeliveryPaymentRequest10DayClient = Pick<
  typeof prisma,
...
export type DeliveryPaymentRequest10DayEventReport = {
  orderType: string;
  orderNumber: string;
  deliveryGroupId: string;
  deliveryDate: string;
...
export type Create10DayDeliveryPaymentRequestEventsSummary = {
  ...
  eventReports: DeliveryPaymentRequest10DayEventReport[];
};

export type Create10DayDeliveryPaymentRequestEventsOptions = {
  runDate?: Date | string;
  dryRun?: boolean;
  prismaClient?: DeliveryPaymentRequest10DayClient;
...
function addSkippedReason(
  summary: Create10DayDeliveryPaymentRequestEventsSummary,
  reason: string
) {
...
export function normalize10DayConfirmVia(value: unknown) {
  if (value === null || value === undefined) return null;
  const trimmed = String(value).trim();
  return trimmed || null;
}

export function requestedOnFor10DayTargetDate(targetDeliveryDate: Date | string) {
  return `${dateKey(targetDeliveryDate)}T${DELIVERY_PAYMENT_REQUEST_10_DAY_REQUESTED_ON_TIME}`;
}

export function is10DayDeliveryGroupEligible(group: DeliveryPaymentRequest10DayTargetGroup) {
  return !(
...
export function get10DayPaymentSkipReason(params: {
...
}): DeliveryPaymentRequest10DaySkipReason | null {
  if (!params.hasOrderTotal) {
    return DELIVERY_PAYMENT_REQUEST_10_DAY_SKIP_REASONS.missingOrderTotal;
  }

  if (!isEligibleDeliveryPaymentTerm(params.paymentTerms)) {
    return DELIVERY_PAYMENT_REQUEST_10_DAY_SKIP_REASONS.paymentTermsNotEligible;
  }

  if (params.unpaidBalance === null || params.unpaidBalance === undefined) {
    return DELIVERY_PAYMENT_REQUEST_10_DAY_SKIP_REASONS.missingUnpaidBalance;
  }
  ...
  if (
    ...
    return DELIVERY_PAYMENT_REQUEST_10_DAY_SKIP_REASONS.noBalanceDue;
  }

  return null;
}

export function get10DayFailedImportExclusions(importResult: ImportSalesOrdersResult) {
  return getFreshImportFailedOrders(importResult);
}

export function isOrderExcludedBy10DayFailedImport(params: {
...
function validateRenderedMessage(params: {
  ...
  if (/\b(null|undefined)\b/i.test(combined) || /:\s*MAIN\s*$/m.test(combined)) {
    throw new Error(
      `Rendered 10-day payment request contains placeholder text order=${params.orderType} ${params.orderNumber}`
    );
  }
}
...
async function createSkippedEvent(params: {
  client: DeliveryPaymentRequest10DayClient;
  deliveryGroup: DeliveryPaymentRequest10DayTargetGroup;
  dedupeKey: string;
  reason: DeliveryPaymentRequest10DaySkipReason;
  dryRun: boolean;
}) {
  if (params.dryRun) {
    return null;
...
export async function create10DayDeliveryPaymentRequestEvents(
  options: Create10DayDeliveryPaymentRequestEventsOptions = {}
): Promise<Create10DayDeliveryPaymentRequestEventsSummary> {
  ...
  const targetDeliveryDate = dateKey(
    getNotificationTargetDate(runDate, DELIVERY_PAYMENT_REQUEST_10_DAY_INTERVAL_DAYS)
  );
  const importRequestedOn = requestedOnFor10DayTargetDate(targetDeliveryDate);
  ...
  if (!deliveryDateSkipReason) {
    ...
    summary.failedImportExclusions = get10DayFailedImportExclusions(summary.importResult);
  }

  const deliveryGroups = await find10DayDeliveryPaymentRequestTargetGroups(
    targetDeliveryDate,
    client
  );
  ...
  for (const deliveryGroup of deliveryGroups) {
    ...
    if (!is10DayDeliveryGroupEligible(deliveryGroup)) {
    ...
    if (
      ...
      isOrderExcludedBy10DayFailedImport({
    ...
    const acumaticaConfirmVia = normalize10DayConfirmVia(order.confirmVia);
    if (!acumaticaConfirmVia) {
      await skipDeliveryGroup({
        ...
        deliveryGroup,
        dedupeKey,
        reason: DELIVERY_PAYMENT_REQUEST_10_DAY_SKIP_REASONS.notConfirmedInAcumatica,
        dryRun,
    ...
    if (channel.selectedChannel === null) {
      await skipDeliveryGroup({
        ...
        deliveryGroup,
        dedupeKey,
        reason: DELIVERY_PAYMENT_REQUEST_10_DAY_SKIP_REASONS.noAutomatedChannelAvailable,
        dryRun,
        acumaticaConfirmVia,
        payment,
        renderedMessagePreview:
          DELIVERY_PAYMENT_REQUEST_10_DAY_SKIP_REASONS.noAutomatedChannelAvailable,
      });
      continue;
    }
