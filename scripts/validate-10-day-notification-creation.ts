...
type Create10DayModule = typeof import("../lib/notifications/create10DayDeliveryPaymentRequestEvents");
    ...
    lines: [
      {
        ...
        orderQty: "1.0000",
        openQty: "1.0000",
        activeAllocatedQty: "1.0000",
        allocationStatus: "allocated",
        etaStatus: "ready",
        readinessStatus: "ready",
        completedQtyDerived: "0.0000",
        payableQuantity: "1.0000",
        payableStockMerchandiseValue: "400.00",
        payableStockTaxAmount: "0.00",
        payableStockTotalValue: "400.00",
        payableBasisIncluded: true,
        payableBasisExclusionReason: null,
...
function targetGroup(overrides: Record<string, unknown> = {}) {
  return {
    ...
    orderType: "SO",
    orderNumber: "SO10",
    deliveryDate: new Date("2026-07-30T00:00:00.000Z"),
    isActive: true,
    lineCount: 1,
...
async function validateMockedRuntimeBehavior(
  failures: string[],
  create10DayModule: Create10DayModule
) {
  const { create10DayDeliveryPaymentRequestEvents } = create10DayModule;
  const globalFlags = { queried: false, dedupeChecked: false };
  try {
    await create10DayDeliveryPaymentRequestEvents({
      runDate: "2026-07-20",
      dryRun: true,
  ...
  const perOrderSummary = await create10DayDeliveryPaymentRequestEvents({
    runDate: "2026-07-20",
    dryRun: true,
  ...
  const weekendSummary = await create10DayDeliveryPaymentRequestEvents({
    runDate: "2026-07-29",
    dryRun: true,
  ...
  assert(
    ...
    "balance greater than the meaningful threshold qualifies as a scheduled dry-run event",
    failures
  );
  assert(
    report?.dedupeKey ===
      "delivery_notification:SO:SO10:2026-07-30:DAY_10:PAYMENT_REQUEST",
    "dedupe key uses DAY_10 and PAYMENT_REQUEST",
    failures
  );
  assert(
    report?.amountDueNowRounded === "170.00",
    "10-day report uses delivery-group-specific amount due",
    failures
  );
  assert(
    report?.paymentDeadlineDate === "2026-07-22",
    "10-day report includes adjusted 8-day deadline",
    failures
  );
}
...
async function main() {
  ...
  const create10DayModule = await import(
  ...
  const {
    ...
    normalize10DayConfirmVia,
  } = create10DayModule;
  const service = read("lib/notifications/create10DayDeliveryPaymentRequestEvents.ts");
  ...
  assertIncludes(service, "NotificationIntervalType.DAY_10", "DAY_10 is used", failures);
  assertIncludes(
    service,
  ...
  assertNotIncludes(
    ...
    "10-day service must not use DELIVERY_REMINDER",
    failures
  );
  assert(
  ...
  assertIncludes(
    ...
    "10-day skip map uses shared weekend delivery-date reason",
    failures
  );
  assertIncludes(
    service,
    "deliveryGroupsSkippedWeekendDeliveryDate",
    "10-day summary reports weekend delivery-date skips",
    failures
  );
  assertIncludes(
    service,
    "detailsLinkId: null",
    "10-day weekend delivery-date dedupe branch detaches details links",
    failures
  );
  assertIncludes(
    service,
  ...
  for (const reason of Object.values(DELIVERY_PAYMENT_REQUEST_10_DAY_SKIP_REASONS)) {
  ...
  assert(normalize10DayConfirmVia(null) === null, "null confirmVia is not confirmed", failures);
  assert(normalize10DayConfirmVia("") === null, "blank confirmVia is not confirmed", failures);
  assert(
    normalize10DayConfirmVia("   ") === null,
    "whitespace confirmVia is not confirmed",
    failures
  );
  assert(
    normalize10DayConfirmVia(" AUTOTXT ") === "AUTOTXT",
  ...
  for (const paymentTerms of [null, "", "   ", "N30NODEP"]) {
    assert(
      get10DayPaymentSkipReason({
        ...
        amountDueNowRounded: "3.00",
        calculationWarnings: [],
      }) === DELIVERY_PAYMENT_REQUEST_10_DAY_SKIP_REASONS.paymentTermsNotEligible,
  ...
  assert(
    ...
    }) === DELIVERY_PAYMENT_REQUEST_10_DAY_SKIP_REASONS.missingOrderTotal,
  ...
  assert(
    get10DayPaymentSkipReason({
      ...
      amountDueNowRounded: null,
    }) === DELIVERY_PAYMENT_REQUEST_10_DAY_SKIP_REASONS.missingUnpaidBalance,
  ...
  assert(
    get10DayPaymentSkipReason({
      ...
      calculationWarnings: [],
    }) === DELIVERY_PAYMENT_REQUEST_10_DAY_SKIP_REASONS.noBalanceDue,
  ...
  assertIncludes(
    ...
    "renderer includes final reminder copy",
    failures
  );
  assertNotIncludes(
    renderer,
    "This is your second balance reminder.",
    "renderer must not include second reminder copy",
    failures
  );
  assertIncludes(
    renderer,
  ...
  for (const forbidden of [
    ...
    assertNotIncludes(service, forbidden, `10-day service must not include ${forbidden}`, failures);
  ...
  await validateMockedRuntimeBehavior(failures, create10DayModule);

  if (failures.length > 0) {
    console.error("10-day notification creation validation failed:");
    for (const failure of failures) console.error(`- ${failure}`);
    process.exit(1);
  }

  console.log(
    "10-day notification creation validation passed. No live SMS/email, Acumatica write, provider dispatch, or DB write was performed."
  );
}
