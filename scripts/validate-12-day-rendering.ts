...
function main() {
  ...
  const params = {
    ...
    detailsLink: "https://mld-delivery.example.test/delivery/details/dd_12_test",
  ...
  const email = render12DayDeliveryPaymentReminderEmail(params);
  const sms = render12DayDeliveryPaymentReminderSms(params);

  assert(
    email.subject ===
      "Balance Reminder: Appliances delivery reminder: Smith Residence - Thursday, July 30, 2026",
    "12-day email subject prefixes existing reminder subject",
    failures
  );
  assertIncludes(
    email.body,
    "Order: SO12",
    "12-day email includes order number",
    failures
  );
  assertIncludes(
    email.body,
    "This is your second balance reminder.",
    "email includes required second balance reminder wording",
    failures
  );
  assertIncludes(
    email.body,
  ...
  for (const [label, source] of [
    ...
    for (const forbidden of [
      ...
      assertNotIncludes(
        source,
        forbidden,
        `12-day ${label} should not include item details: ${forbidden}`,
        failures
      );
    }
  }
  assertIncludes(
    email.body,
    "Review delivery details here: https://mld-delivery.example.test/delivery/details/dd_12_test",
  ...
  assertIncludes(
    sms,
    "MLD: Order SO12:",
    "12-day SMS includes order number",
    failures
  );
  assertIncludes(
    sms,
    "Balance may be due before delivery. Please review details here: https://mld-delivery.example.test/delivery/details/dd_12_test.",
  ...
  if (failures.length > 0) {
    console.error("12-day rendering validation failed:");
    for (const failure of failures) console.error(`- ${failure}`);
    process.exit(1);
  }

  console.log("12-day rendering validation passed.");
}
